...
def horizontal_flip(img, bboxes):
    ...
    for bbox in bboxes:
        old =bbox[2]
        bbox[2] = w_img - int(bbox[0])
        bbox[0] = w_img - int(old)    
...
def readAnnotations(xml_path):
    ...
    return results
def writeXml1(tmp, imgname, w, h, d, bboxes,temp2):
        xmlname = os.path.splitext(imgname)[0]
        tempfile = temp2 + "/%s.xml" % xmlname
        
        # gt, cls_id = convert_annotation(src_txt_dir, img)
        gt=bboxes
        xml_file = open((tempfile), 'w')
        xml_file.write('<?xml version="1.0" ?>\n')
        xml_file.write('<doc>\n')
        xml_file.write(f'<folder>{tmp}</folder>\n')
        xml_file.write(f'<filename>{imgname}</filename>\n')
        xml_file.write('    <path>' +tmp+'\\'+imgname + '</path>\n')
        ...
        for spt in gt:
            xml_file.write('            <item>\n')
            xml_file.write(f'                <name>{spt[4]}</name>\n')
        ...
        xml_file.write('        <depth>'+str(d)+'</depth>\n')
        xml_file.write('    </size>\n')
        xml_file.write('</doc>')
    
        # print(imgname,"go wrong")
...
def writeXml(tmp, imgname, w, h, d, bboxes):
    ...
    database_txt = doc.createTextNode("Strawberry")
...
def resize2416(img, bboxes):
    IMAGE_SIZE=416
    w = img.shape[1]
    h = img.shape[0]
    scalewidth = IMAGE_SIZE / w
    scaleheight = IMAGE_SIZE / h

    results=[]
    for box in bboxes:
        result=[]
        x1,y1,x2,y2,classname=box[0],box[1],box[2],box[3],box[4]
        newx1 = int(int(x1) * scalewidth)
        newy1 = int(int(y1) * scaleheight)
        newx2 = int(int(x2) * scalewidth)
        newy2 = int(int(y2) * scaleheight)

        result.append(int(newx1))
        result.append(int(newy1))
        result.append(int(newx2))
        result.append(int(newy2))
        result.append(classname)
        print(*result)

        results.append(result)
    
    imgnew=cv2.resize(img,(IMAGE_SIZE,IMAGE_SIZE))
    
    return imgnew , results
...
if __name__ == "__main__":
    root = ''
    img_dir = root + 'image'
    anno_path = root + 'voc_annotation'
    # print(img_dir)
    # 设置数据扩增的方式
    Method = 'traditional5'
    # method_sets = ['random_horizontal_flip', 'random_translate', 'random_crop', 'random_bright']
    
    # 存储新的anno位置
    anno_new_dir = os.path.join(root, Method, 'voc_annotation')
    if not os.path.isdir(anno_new_dir):
        os.makedirs(anno_new_dir)
 
    # 扩增后图片保存的位置
    img_new_dir = os.path.join(root, Method, 'image')
    ...
    for image_path in img_list:
        ...
        #平移
        new_img, new_bboxes = _shift_pic_bboxes(new_img, new_bboxes)
 
        #裁剪
        new_img, new_bboxes = _crop_img_bboxes(new_img, new_bboxes)
 
        #亮度
        new_img, new_bboxes = _changeLight(new_img, new_bboxes)
            
        #高斯噪声
        new_img, new_bboxes = _addNoise(new_img, new_bboxes)
 
        #旋转
        new_img, new_bboxes = _rotate_img_bboxes(new_img, new_bboxes)
 
        #镜像
        new_img, new_bboxes = horizontal_flip(new_img, new_bboxes)

        #RESIZE 2 416*416
        new_img, new_bboxes = resize2416(new_img, new_bboxes)
        ...
        writeXml1(img_new_dir, new_img_name, W, H, D, new_bboxes,anno_new_dir)
        # img = np.array(img)
        # for box in bboxes:
